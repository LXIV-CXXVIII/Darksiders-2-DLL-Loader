...
#define compile_seed (	(__TIME__[7] - '0') * 1  + (__TIME__[6] - '0') * 10		+	\
	...
	constexpr int LinearCongruentGenerator(int Rounds) {
		return (long long)1013904223 + (long long)1664525 * ((Rounds > 0) ? LinearCongruentGenerator(Rounds - 1) : compile_seed & 0xFFFFFFFF);
	}
...
#define RandomNumber(Min, Max) (Min + (Random() % (Max - Min + 1)))
	...
	private:
		...
		static const Char XORKEY = static_cast<Char>(RandomNumber((short)0, (short)0xFFFF));
